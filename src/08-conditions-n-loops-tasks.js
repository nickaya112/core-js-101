...
function doRectanglesOverlap(rect1, rect2) {
  ...
  || (top2 >= bottom1 && right2 >= left1 && top2 <= top1 && right2 <= top2);
}
...
function isInsideCircle(circle, point) {
  ...
  const allLength = Math.sqrt(x ** 2 + y ** 2);
  if (allLength < circle.radius) {
    return true;
  }
  return false;
}
...
function findFirstSingleChar(str) {
  const arrChars = [];
  const arrSort = str.split('').sort();
  arrSort.forEach((item, index) => {
    if (arrSort[index] !== arrSort[index + 1] && arrSort[index] !== arrSort[index - 1]) {
      arrChars.push(arrSort[index]);
    }
  });
  if (arrChars.length === 0) return null;
  return arrChars[0];
}
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    let cardNumber = parseInt(arr[i], 10);
    if ((arr.length - i) % 2 === 0) {
      cardNumber *= 2;
      if (cardNumber > 9) {
        cardNumber -= 9;
...
function getDigitalRoot(num) {
  let counter = String(num).split('').reduce((sum, item) => sum + +item, 0);
  while (counter > 9) {
    counter = String(counter).split('').reduce((sum, item) => sum + +item, 0);
  }
  return counter;
}
...
function isBracketsBalanced(str) {
  const brackets = [['(', ')'], ['[', ']'], ['{', '}'], ['|', '|'], ['<', '>']];
  const openBrackets = [str[0]];
  for (let i = 1; i < str.length; i += 1) {
    const lastElem = openBrackets[openBrackets.length - 1];
    let inverseElem;
    brackets.forEach((item) => {
      if (item[0] === lastElem) {
        const replaceEl = item[1];
        inverseElem = replaceEl;
      }
    });
...
function getCommonDirectoryPath(pathes) {
  const firstPart = pathes[0];
  let commonDirectoryPath = '';
  for (let i = 0; i < firstPart.length; i += 1) {
    const char = firstPart[i];
    for (let j = 1; j < pathes.length; j += 1) {
      if (char !== pathes[j][i]) {
        return commonDirectoryPath.slice(0, commonDirectoryPath.lastIndexOf('/') + 1);
      }
    }
    commonDirectoryPath += char;
  }
...
function getMatrixProduct(m1, m2) {
  ...
  const numColsM1 = m1[0].length;
  const numRowsM2 = m2.length;
  const numColsM2 = m2[0].length;
  const newMatrix = [];
  ...
  for (let index = 0; index < numColsM2; index += 1) {
    for (let i = 0; i < numRowsM1; i += 1) {
      let elemMatrix = 0;
      for (let j = 0; j < numRowsM2; j += 1) {
        elemMatrix += m1[i][j] * m2[j][index];
      }
      newMatrix[i][index] = elemMatrix;
    }
  }
  return newMatrix;
}
